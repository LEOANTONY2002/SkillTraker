...
function Profile() {
  ...
  const update = async () => {
    try {
      const { data } = await Axios.put(
        "/user/skill",
        { email: user.email, skills: userSkill },
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );
  ...
  const addSkill = async (s, exp) => {
    console.log("add", s);
    let check = await userSkill.find((us) => us.skill === s._id);
    console.log(check);
    if (!check) {
      await setSkill({
        ...skill,
        skills: [...skill.skills, { skill: s, exp }],
      });
                  ...
                  skill.skills.map((s) => (
                    <div
                      ...
                      style={
                        s?.exp !== "Beginner"
                          ? s?.exp === "Intermediate"
                            ? { backgroundColor: "#ffbfac", color: "white" }
                            : { backgroundColor: "#ff815a", color: "white" }
                          : { backgroundColor: "#fde5e5", color: "white" }
                      }
                      className="sab-sk"
                    >
                      <p>{s?.skill?.name}</p>
                  ...
                  skills.map((s) => (
                    <div className="sab">
                      ...
                      <section className="sab-exp">
                        ...
                        <p onClick={() => addSkill(s, "Intermediate")}>
                          Intermediate
                        </p>
